// performer-setup.js - Save to Supabase with display name length check
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Character counter for stage name
    const stageNameCounter = document.getElementById('stageNameCounter');
    stageNameInput.addEventListener('input', function() {
        stageNameCounter.textContent = this.value.length + '/20';
    });
    ...
    form.addEventListener('submit', async function(e) {
        ...
        setTimeout(() => window.location.href = 'performer-hub.html', 1500);
    });

    // Update review on input
    stageNameInput.addEventListener('input', updateReview);
    locationInput.addEventListener('input', updateReview);
    ...
    function updateReview() {
        ...
        if (reviewName) reviewName.textContent = stageNameInput?.value.trim() || 'Stage Name';
        if (reviewLocation) reviewLocation.textContent = locationInput?.value.trim() || 'Location';
        if (reviewTypes) {
            if (window.selectedTypes.length > 0) {
                reviewTypes.innerHTML = window.selectedTypes.map(t => `<span class="type-badge-small">${t}</span>`).join(' ');
            } else {
        ...
        if (reviewPic) {
            ...
            if (img) {
                reviewPic.innerHTML = `<img src="${img.src}" style="width:100%; height:100%; object-fit:cover; border-radius:50%;">`;
            } else if (window.profileImageData) {
                reviewPic.innerHTML = `<img src="${window.profileImageData}" style="width:100%; height:100%; object-fit:cover; border-radius:50%;">`;
            } else {
    ...
    function validateStep(step) {
        ...
        switch(step) {
            case 1:
                ...
                if (name.length < 2) {
                    showError('nameError', 'Stage name must be at least 2 characters');
                    return false;
                }
                if (name.length > 20) {
                    showError('nameError', 'Stage name cannot exceed 20 characters');
                    return false;
                }
    ...
    function goToStep(step) {
        ...
        if (progressFill) progressFill.style.width = `${((step-1)/2)*100}%`;
...
function showToast(message, type = 'success') {
    ...
    toast.style.cssText = `
        ...
        color: white; border-radius: 10px; z-index: 10001;
        animation: slideInRight 0.3s ease; box-shadow: 0 5px 15px rgba(0,0,0,0.2);
    `;
