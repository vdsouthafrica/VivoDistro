...
class NotificationManager {
    constructor(sb, userId) {
        ...
        this.autoPromptScheduled = false;
    ...
    async init() {
        ...
        if (!this.boundPermissionRequestHandler) {
            this.boundPermissionRequestHandler = () => this.requestPermission();
            window.addEventListener('requestNotificationPermission', this.boundPermissionRequestHandler);
        }
        ...
        if (Notification.permission === 'default' && !this.autoPromptScheduled) {
            this.autoPromptScheduled = true;
    ...
    async subscribeToPush() {
        if (this.subscribeInFlight) return;
    ...
    async syncSubscriptionWithServer(subscription) {
        ...
        const subscriptionPayload = typeof subscription.toJSON === 'function'
            ? subscription.toJSON()
            : subscription;
        const endpoint = subscriptionPayload?.endpoint;

        if (!endpoint) {
            console.error('❌ PUSH SYNC ERROR: Subscription endpoint is missing.');
            return;
        }

        const { data: existingRows, error: fetchError } = await this.sb
            .from('push_subscriptions')
            .select('id, subscription')
            .eq('user_id', this.userId);

        if (fetchError) {
            console.error('❌ PUSH SYNC FETCH ERROR:', fetchError.message, fetchError.details, fetchError.hint);
            return;
        }

        const duplicateRows = (existingRows || []).filter(row => row.subscription?.endpoint === endpoint);
        for (const row of duplicateRows) {
            ...
            if (deleteError) {
                console.error('❌ PUSH SYNC DELETE ERROR:', deleteError.message, deleteError.details, deleteError.hint);
                return;
            }
        }
        ...
        if (insertError) {
            console.error('❌ PUSH SYNC INSERT ERROR:', insertError.message, insertError.details, insertError.hint);
        } else {
    ...
    setupReminders() {
        ...
        this.reminderIntervalId = setInterval(() => this.checkAllUpcomingEvents(), 60000);
    }
